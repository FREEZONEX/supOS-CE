...
import { deleteTreeNode, detectIfRemoveApi } from '@/apis/core-api/uns';
      ...
      {deleteDetail?.pathType === 2 ? (
        <div style={{ display: 'flex', gap: '5px', fontSize: '16px' }}>
          ...
          <Flex vertical justify="flex-start">
            <span>{formatMessage('uns.deleteFileRecycleTip')}</span>
            {showConfirm && <span>{formatMessage('uns.thisNodeHasAssociatedComputingNodes')}</span>}
          </Flex>
        </div>
      ) : (
        <div style={{ display: 'flex', gap: '5px', fontSize: '16px' }}>
