...
func (h *MenuHandler) SaveMenuHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if err := r.ParseMultipartForm(32 << 20); err != nil { // 32MB max memory
			httpx.Error(w, errors.NewBuzError(r.Context(), 500, "request.parse.failed"))
			return
		}

		openType, err := strconv.Atoi(r.FormValue("openType"))
		if err != nil {
			httpx.Error(w, errors.NewBuzError(r.Context(), 500, "menu.opentype.invalid"))
			return
		}

		if err := validator.ValidateOpenType(r.Context(), openType); err != nil {
			httpx.Error(w, errors.NewBuzError(r.Context(), 500, err.Error()))
			return
		}
		...
		if err == nil {
			...
			httpx.Error(w, errors.NewBuzError(r.Context(), 500, "menu.icon.read.failed"))
			return
		}
		...
		if err := h.menuLogic.CreateMenu(r.Context(), menuDto, true); err != nil {
			httpx.Error(w, err)
			return
		}
