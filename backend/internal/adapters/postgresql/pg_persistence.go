	...
	for i := 0; i < len(tableInfo.data); i += batchSize {
		...
		for j, record := range batch {
			...
			for k, f := range tableInfo.def.Fields {
				v, has := record[f.Name]
				if !has {
					row[k] = types.FieldType(f.Type).DefaultValue()
					continue
				}
...
// 带重试的版本
/*func (p *PgPersistentService) PersistenceWithRetry(unsData []service.UnsData) error {
	var lastError error

	for attempt := 1; attempt <= p.maxRetries; attempt++ {
		err := p.persistence(unsData)
		if err == nil {
			return nil
		}

		lastError = err
		p.log.Debugf("持久化失败 (尝试 %d/%d): %v", attempt, p.maxRetries, err)
	...
	return fmt.Errorf("持久化最终失败: %v", lastError)
}
*/
