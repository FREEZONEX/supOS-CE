...
func (l *GetGroupedEventFlowListLogic) GetGroupedEventFlowList(req *types.GroupPageRequest) (resp *types.EventFlowGroupPageResp, err error) {
	...
	pageResult := &types.EventFlowGroupPageResp{
	...
	l.Logger.Debugf("GetGroupedEventFlowList: PageList request: %+v", req)
	db := relationDB.GetDb(l.ctx)

	// 调用DAO层方法获取分组后的event flow列表
	items, total, err := l.sourceFlowRepo.GetGroupedEventFlowList(db, req)
	if err != nil {
		l.Logger.Error("查询分组event flow列表失败:", err)
		return nil, err
	}
