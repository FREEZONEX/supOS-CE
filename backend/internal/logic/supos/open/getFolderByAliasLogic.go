...
func (l *GetFolderByAliasLogic) GetFolderByAlias(req *types.AliasPathReq) (resp *types.ResultVO, err error) {
	// 获取 uns 查询服务
	unsQueryService := spring.GetBean[*uns.UnsQueryService]()

	// 根据别名获取文件夹详情
	modelDefinition, err := unsQueryService.GetModelDefinition(l.ctx, &types.ModelDetailReq{}, req.Alias, "")
	if err != nil {
