...
type KeycloakConfig struct {
	...
	ClientSecret           string
	RedirectURI            string
	SuposClientID          string // The internal ID of the 'supos' client
...
func (kc *KeycloakClient) GetAdminToken() (string, error) {
	if token, found := kc.tokenCache.Get(adminTokenCacheKey); found {
	...
	kc.tokenCache.Set(adminTokenCacheKey, tokenResp.AccessToken, time.Duration(tokenResp.ExpiresIn-60)*time.Second)
